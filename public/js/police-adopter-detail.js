const AR_ID = window.location.pathname.split('/').pop();

async function loadDetail() {
  const messageBox = document.getElementById('message');
  try {
    const res = await fetch(`/api/police/adopters/${AR_ID}`);
...
async function verify(status) {
  ...
  if (!report.trim()) {
    messageBox.className = 'message error';
    messageBox.textContent = 'A report is required before you can verify or reject this adopter.';
    return;
  }

  try {
    const res = await fetch('/api/police/verify-adopter', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ AR_ID: Number(AR_ID), status, report })
    });
    const data = await res.json();
